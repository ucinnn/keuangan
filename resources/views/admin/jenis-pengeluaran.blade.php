<x-layout-admin>
    ...
    <body class="bg-gray-100">
        <div class="flex h-screen flex-col">
            ...
            <div class="flex-grow bg-white p-6">
                <div class="mb-6">
                    <div class="text-lg font-bold">Manajemen Jenis pengeluaran</div>
                </div>
                <div class="flex items-center mb-4 justify-between">
                    <div class="border border-gray-300 p-3 rounded flex items-center">
                        <form method="GET" action="{{ route('admin.filter-jenis-pengeluaran') }}">
    ...
    <option value="9" {{ request('unitpendidikan') == '9' ? 'selected' : '' }}>Tidak Pondok</option>
                    ...
                    <a href="{{ route('admin.create-jenis-pengeluaran') }}">
                        <button class="bg-green-500 text-white p-2 rounded">+ Tambah Data Pengeluaran</button>
                    </a>
                </div>
                    ...
                    <table id="data-table" class="min-w-full bg-white border border-gray-300">
                        <thead>
                            ...
                            <tr class="bg-gray-200 text-gray-700">
                                ...
                                <th class="py-1 px-4 border-b border-r border-gray-300 text-left">Nama Pengeluaran</th>
                                <th class="py-1 px-4 border-b border-r border-gray-300 text-left">Tipe Pengeluaran</th>
                        ...
                        <tbody>
                            @foreach ($filtered_data as $item)
                                <tr class="hover:bg-gray-50 transition">
                                    ...
                                    <!-- Gabungkan Nama pengeluaran dengan Unit -->
                                    <td class="border px-4 py-2 text-left">
                                    {{ $item->namaUnit }} - {{ $item->nama_pengeluaran }}</td>
                                    ...
                                    <td class="border px-4 py-2 text-left">{{ $item->nominal_jenispengeluaran }}</td>
                                    <td class="border px-4 py-2 text-left">{{ $item->status }}</td>
                                    <td class="border px-4 py-2 text-left">
                                        <a
                                            href="{{ route('admin.edit-jenis-pengeluaran', ['id' => $item->id]) }}">
